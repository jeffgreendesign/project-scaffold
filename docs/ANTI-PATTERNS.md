...
| Implementing multi-file changes without a plan | Agent misunderstands a requirement, builds 8 files in the wrong direction, you revert everything. More capable models go further, faster, on wrong assumptions. | For changes touching 3+ files or introducing new patterns, use `/design` (Claude) or the plan-first working rule (Codex/Gemini) to research and propose before implementing. Review the proposal. Then approve. This is the **Orchestrator-Subagent** coordination pattern (research → propose → approve → implement). |
| Reaching for agent teams, message buses, or shared-state loops before trying orchestrator-subagent | The coordination-patterns blog (2026-04-10) recommends starting with orchestrator-subagent — widest range of problems, least overhead. Jumping to parallel workers or event buses adds coordination cost without clear justification. | Start with the plan-first workflow (Claude: `/design`; Codex/Gemini: AGENTS.md rule 5). Evolve only when it demonstrably struggles. See `docs/COORDINATION-PATTERNS.md`. |
